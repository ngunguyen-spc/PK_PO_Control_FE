...
//       //Check lại có cần thiết không?
//       final List<RemainTableDetailIDModel> detailsID =
//         await ApiService().fetchRemainTableDetailMTDID(widget.div, date, 'All', 'All');
//
//       overlay.remove();
//       if (!mounted) return;
//
...
//                                         color: isTouched ? (isDark ? Colors.white : Colors.black) : Colors.black87,
//                                     ),
//                                   ),
//                                 )
//                                     : Text(
...
class _RemainChartWidgetState extends State<RemainChartWidget>
  ...
  // ── Click bar → fetch PO detail theo ngày → mở popup ────────────────────
  // ✅ KHÔNG fetch MTD_ID ở đây — popup sẽ lazy load khi user toggle tab ID
  void _onBarTap(int index) async {
    if (index < 0 || index >= widget.data.length) return;
    final d    = widget.data[index];
    final date = d.date; // 'yyyy-MM-dd'
    ...
    try {
      // ✅ Chỉ fetch PO detail theo ngày click (daily, không phải MTD)
      final List<RemainTableDetailModel> details =
      await ApiService().fetchRemainTableDetail(widget.div, date, 'All', 'All');
      ...
      showDialog(
        ...
        builder: (_) => RemainTableDetailPopup(
          ...
          title: '${widget.div} · $dateLabel',
          data: details,
          dataID: const [], // ✅ Rỗng — popup lazy load khi user toggle tab ID
          div: widget.div,
          date: date,       // ✅ Truyền date để popup biết fetch MTD_ID theo ngày nào
          cusID: '',
          shipBy: '',
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    final labels = widget.data.map((e) {
      try {
        final dt = DateTime.parse(e.date);
        return '${dt.day}/${dt.month}';
      } catch (_) {
                      ...
                      sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (value, meta) {
                          ...
                          if (i < 0 || i >= widget.data.length)
                            return const SizedBox.shrink();
                          final d = widget.data[i];
                          if (d.remain_PO == 0) return const SizedBox.shrink();

                          final pct = d.ex_PO > 0
                              ? (d.remain_PO / d.ex_PO * 100).toStringAsFixed(0)
                              : '0';
                            ...
                            children: [
                              ...
                              Text(
                                ...
                                style: TextStyle(
                                  fontSize: 14,
                                  color: isDark
                                      ? AppColors.remainDark.withOpacity(0.8)
                                      : AppColors.remainLight.withOpacity(0.8),
                                ),
                              ),
                            ],
                          );
