...
//       setState(() => _hasInput = _filterController.text.trim().isNotEmpty);
//       _applyFilter();
//     });
//   }
//
...
//       widget.div, dateStr, widget.cusID, widget.shipBy,
//     );
...
//   // ── Filter helpers ────────────────────────────────────────────────────────
//   bool _checkHasInput() =>
//       selSsd != null || selCusID != null || selShipBy != null ||
//           selDenk != null || selVbeln != null || selPo != null || selDiv != null ||
//           selFerth != null || selRoname != null || selExQty != null ||
//           selFnQty != null || selRemainQty != null ||
...
//           final searchable = ['ssd','cusID','shipBy','denk','vbeln','po','div','ferth','roname']
...
//       selSsd = selCusID = selShipBy = selDenk = selVbeln = selPo =
//           selDiv = selFerth = selRoname = selExQty = selFnQty = selRemainQty = null;
...
//                       onPressed: () => setState(() => _colWidths = List.from(_defaultWidths)),
//                     ),
//                   ),
//                 ],
//               ),
...
//             Row(
//               children: [
//                 _statChip('Total PO',   '${filteredData.length}',       Colors.white,      isDark),
//                 const SizedBox(width: 10),
//                 _statChip('Total Qty',  fmt.format(totalExQty),         Colors.white,      isDark),
//                 const SizedBox(width: 10),
...
//                 onPressed: () { _filterController.clear(); _applyFilter(); })
//                 : null,
...
//         ? (isDark ? AppColors.remainBgDark.withValues(alpha: 0.15) : AppColors.remainBgLight.withValues(alpha: 0.3))
...
//                 color: headerBg,
//                 child: Row(
//                   children: List.generate(_colDefs.length, (i) =>
//                       _buildHeaderCell(
...
//       String label, String key, int colIdx,
//       bool isNumber, Color borderColor, bool isDark,
//       ) {
//     final values = <String>{};
...
//               ),
//               searchable: true,
//               items: (values.toList()..sort()).map((v) => MultiSelectItem(v, v)).toList(),
//               initialValue: _getSelected(key),
//               buttonIcon: Icon(Icons.keyboard_arrow_down_outlined, size: 16,
...
//               confirmText: const Text('OK', style: TextStyle(color: Colors.blueAccent, fontWeight: FontWeight.bold)),
...
//       decoration: BoxDecoration(
//           color: rowBg, border: Border.all(color: borderColor, width: 0.5)),
//       alignment: Alignment.centerLeft,
//       child: Text((j[key] ?? '').toString(),
...
//
//     return Row(children: [
//       txtCell('ssd',    0), txtCell('cusID',  1), txtCell('shipBy', 2),
//       txtCell('denk',   3), txtCell('vbeln',  4), txtCell('po',     5),
//       txtCell('div',    6), txtCell('ferth',  7), txtCell('roname', 8),
...
//     sheet.appendRow([
//       'SSD','CusID','ShipBy','DENK','VBELN','PO',
//       'Div','FERTH','RONAME','Ex Qty','Fn Qty','Remain Qty',
//     ]);
//     for (final item in data) {
//       final j = item.toJson();
//       sheet.appendRow([
//         j['ssd'], j['cusID'], j['shipBy'], j['denk'],
...
fontSize: 14, color: Colors.black87)))
    : SelectableText(value == 0 ? '-' : fmt.format(value),
style: TextStyle(fontSize: 14,
color: value == 0 ? Colors.grey.shade500 : null));
